...
bot.action('cek_saldo', async (ctx) => {
  try {
    ...
    if (row) {
      await ctx.reply(`📊 *Cek Saldo*\n\n🆔 ID Telegram: ${userId}\n💰 Sisa Saldo: Rp${row.saldo}`, { parse_mode: 'Markdown' });
    } else {
  ...
  try {
    ...
    const inlineKeyboard = [
      [
        {
          text: "Batal Top-Up",
          callback_data: `batal_topup:${uniqueCode}`
        }
      ]
    ];
...
// Fungsi untuk membatalkan top-up
async function batalTopup(ctx, uniqueCode) {
  const userId = ctx.from.id;

  // Cek apakah uniqueCode ada dalam pendingDeposits
  if (!global.pendingDeposits || !global.pendingDeposits[uniqueCode]) {
    await ctx.reply('⚠️ *Permintaan pembatalan tidak dapat diproses. Tidak ada top-up yang ditemukan dengan kode unik tersebut.*', { parse_mode: 'Markdown' });
    return;
  }

  // Ambil informasi deposit terkait
  const depositInfo = global.pendingDeposits[uniqueCode];

  // Hapus dari global.pendingDeposits
  delete global.pendingDeposits[uniqueCode];

  // Hapus dari database
  db.run('DELETE FROM pending_deposits WHERE unique_code = ?', [uniqueCode], (err) => {
    if (err) {
      logger.error('Gagal hapus pending_deposits:', err.message);
    }
  });

  await ctx.reply(`✅ *Top-up yang menggunakan kode unik: ${uniqueCode} berhasil dibatalkan.*`, { parse_mode: 'Markdown' });
}
...
bot.on('callback_query', async (callbackQuery) => {
  ...
  if (data.startsWith('batal_topup:')) {
    const uniqueCode = data.split(':')[1];
    await batalTopup({ from: callbackQuery.from, reply: (msg) => bot.sendMessage(chatId, msg), replyWithPhoto: (opts) => bot.sendPhoto(chatId, opts) }, uniqueCode);
    await bot.answerCallbackQuery(callbackQuery.id); // Menghapus tanda loading pada tombol
  }
});
