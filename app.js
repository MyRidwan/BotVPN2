...
async function sendMainMenu(ctx) {
  ...
  let jumlahServer = await getCountFromDB('Server');
  let jumlahPengguna = await getCountFromDB('users');
...
// Utility function to get count from database
async function getCountFromDB(tableName) {
  try {
    ...
    logger.error(`Kesalahan saat mengambil jumlah dari ${tableName}:`, err.message);
...
async function sendAdminMenu(ctx) {
  ...
  if (error.response && error.response.error_code === 400) {
    await ctx.reply(`*Selamat datang di ${NAMA_STORE},\nMENU BOT ADMIN* 🚀\n__Bot VPN serba otomatis untuk membeli\nlayanan VPN dengan mudah dan cepat__\n\n🪪 *Tele:* @kytxz\n🍀 *Wa:* 6285691670272\n\n⏳ *Uptime bot:* ${days} Hari\n🌐 *Server tersedia:* ${jumlahServer}\n👥 *Jumlah pengguna:* ${jumlahPengguna}\n\n*Silakan pilih opsi layanan:*`, {
      reply_markup: {
        inline_keyboard: adminKeyboard
      }
    });
  ...
  try {
    ...
    const inlineKeyboard = [
      [
        {
          text: "🛠 Hubungi Admin",
          url: "https://t.me/kytxz"  // Ganti callback_data dengan url
        }
      ]
    ];
...
async function batalTopup(ctx, uniqueCode) {
  // Internal handling could be omitted since now we are not managing deposits cancelation via button
}
