...
bot.on('callback_query', async (callbackQuery) => {
    ...
    if (data.startsWith('batal_topup:')) {
        ...
        logger.info(`Menerima permintaan batal dengan kode unik: ${uniqueCode}`);
        
        await batalTopup({ 
            from: callbackQuery.from, 
            reply: (msg) => bot.sendMessage(chatId, msg), 
            replyWithPhoto: (opts) => bot.sendPhoto(chatId, opts) 
        }, uniqueCode);
        
        await bot.answerCallbackQuery(callbackQuery.id);
    } else {
        logger.warn(`Data callback tidak dikenal: ${data}`);
    }
});
