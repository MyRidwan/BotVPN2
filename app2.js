...
async function processDeposit(ctx, amount) {
  ...
  try {
    ...
    const caption =
      ...
      `🔄 Untuk membatalkan top-up, kirim pesan: /batal_topup ${uniqueCode}`;
...
bot.on('text', async (msg) => {
    ...
    // Memeriksa apakah pesan adalah perintah batal top-up
    if (text.startsWith('/batal_topup')) {
        const uniqueCode = text.split(' ')[1];
        logger.info(`Menerima permintaan batal dengan kode unik: ${uniqueCode}`);

        // Memanggil fungsi batalTopup
        await batalTopup(
            {
                from: msg.from,
                reply: (responseMsg) => bot.sendMessage(chatId, responseMsg), 
